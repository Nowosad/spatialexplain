...
It has a function called `predict_spatial_parts()` that can be used to calculate the attributions of the model (Break Down, SHAP, or Oscillations explanations) for each observation in the raster.
The function requires the explainer object and the predictor's raster.
Additionally, as the calculation of the attributions can be computationally expensive, the function has a `maxcell` parameter that controls the number of cells in the raster that will be used to get the attributions.
...
By default, the `predict_spatial_parts()` function calculates the Break Down attributions.
...
Finally, we can calculate the attributions for the classification model using the `predict_spatial_parts()` function.
